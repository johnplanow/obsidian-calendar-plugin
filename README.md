...
❗❗❗ Regarding this fork at https://github.com/johnplanow/obsidian-calendar-plugin

This custom version includes new settings:
* Daily notes will be pinned after opening. *Default*: disabled
* Daily notes will open in the pane containing a note with the specified tag. *Default*: disabled
* This is useful if you have a specific pane for your daily notes and prefer they always open there.
  * WARNING: This feature has not been tested extensively with multiple notes open containing the specified tag. The behavior may not be intuitive.
  
To install it, clone the repo into your vault's plugin directory (`<vault>/.obsidian/plugins`), then run `npm install` and `npm run build`. You'll then need to force Obsidian to reload (View -> Force Reload) and enable the plugin. Be sure to disable the original Calendar plugin if you haven't.
